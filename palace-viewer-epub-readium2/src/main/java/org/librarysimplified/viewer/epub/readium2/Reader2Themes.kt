...
object Reader2Themes {
  ...
  private fun fromSR2PublisherCSS(publisherCSS: SR2PublisherCSS): ReaderPublisherCSS =
    when (publisherCSS) {
  ...
  private fun fromSR2Size(textSize: Double): Double = textSize * 100.0

  private fun fromSR2Font(font: SR2Font): ReaderFontSelection =
    when (font) {
      SR2Font.FONT_SANS -> {
  ...
  private fun fromSR2Color(colorScheme: SR2ColorScheme): ReaderColorScheme =
    when (colorScheme) {
  ...
  fun toSR2(readerPreferences: ReaderPreferences): SR2Theme =
    SR2Theme(
  ...
  private fun toSR2PublisherCSS(publisherCSS: ReaderPublisherCSS): SR2PublisherCSS =
    when (publisherCSS) {
  ...
  private fun toSR2Size(fontScale: Double): Double = SR2Theme.sizeConstrain(fontScale / 100.0)
